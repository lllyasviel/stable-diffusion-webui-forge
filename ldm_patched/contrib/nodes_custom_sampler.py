...
class BetaSamplingScheduler:
    ...
    def get_sigmas(self, model, steps, alpha, beta):
        sigmas = ldm_patched.modules.samplers.beta_scheduler(model.get_model_object("model_sampling"), steps, alpha=alpha, beta=beta)
        return (sigmas, )
...
class SamplerDPMPP_2S_Ancestral:
    ...
    def get_sampler(self, eta, s_noise):
        sampler = ldm_patched.modules.samplers.ksampler("dpmpp_2s_ancestral", {"eta": eta, "s_noise": s_noise})
        return (sampler, )
...
class SamplerCustomAdvanced:
    ...
    def sample(self, noise, guider, sampler, sigmas, latent_image):
        ...
        latent_image = latent["samples"]
        latent = latent.copy()
        latent_image = ldm_patched.modules.sample.fix_empty_latent_channels(guider.model_patcher, latent_image)
        latent["samples"] = latent_image
        ...
        x0_output = {}
        callback = ldm_patched.utils.latent_visualization.prepare_callback(guider.model_patcher, sigmas.shape[-1] - 1, x0_output)
