...
class BertEmbeddings(torch.nn.Module):
    ...
    def forward(self, input_tokens, token_type_ids=None, dtype=None):
        ...
        x += ldm_patched.modules.ops.cast_to_input(self.position_embeddings.weight[:x.shape[1]], x)
        if token_type_ids is not None:
            x += self.token_type_embeddings(token_type_ids, out_dtype=x.dtype)
        else:
            x += ldm_patched.modules.ops.cast_to_input(self.token_type_embeddings.weight[0], x)
        x = self.LayerNorm(x)
        return x
